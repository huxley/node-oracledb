...
// This version of node-oracledb works with Node.js 14.6 or later or Bun 1.2+.
...
if (process.versions.bun) {
  // Parse Bun version (e.g. "1.2.3") and ensure it’s 1.2 or greater.
  const bunVs = process.versions.bun.split(".").map(Number);
  errors.assert(
    bunVs[0] > 1 || (bunVs[0] === 1 && bunVs[1] >= 2),
    errors.ERR_NODE_TOO_OLD,
    nodbUtil.PACKAGE_JSON_VERSION,
    "1.2"
  );
} else {
  // Existing check for Node.js 14.6 or later.
